...
def infer_relations(text: str) -> List[str]:
    ...
    if any(k in t for k in ["adverse effect", "toxicity", "side effect", "adverse event"]):
...
def inject_reasoning(output_path: str, kg: KGMultiHop, max_hops: int = 3):
    ...
    for row in rows:
        ...
        explanations = [" -> ".join(f"{src} -[{rel}]-> {tgt}" for src, rel, tgt in path) for path in paths]
...
def main():
    ...
    import sys
    cmd = [
        sys.executable, "scripts/run_hybrid.py",
        "--corpus", args.corpus,
        "--kg", args.kg_csv,
        "--dict", args.dict,
